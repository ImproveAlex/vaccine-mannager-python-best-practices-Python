...
class TestGetVaccineDate(TestCase):
    ...
    # add a patient in the store
        ...
        # Fecha para pasar como argumento
        # Ya que antes utilizabamos days = 10, para no cambiar el hash simplemente hemos
        # hecho que la fecha de vacunacion sea 10 dias despues
        vac_date = "2022-03-18"
    #check the method
        value = my_manager.get_vaccine_date(file_test,vac_date)
        ...
        for file_name,expected_value in param_list_nok:
            with self.subTest(test=file_name):
                ...
                with self.assertRaises(VaccineManagementException) as c_m:
                    my_manager.get_vaccine_date(file_test,vac_date)
    ...
    def test_wrong_date(self):
        """la fecha de vacunacion introducida es previa a la fecha actual"""
        file_test = JSON_FILES_RF2_PATH + "test_ok.json"
        vac_date = "2022-03-01"
        my_manager= VaccineManager()
        exception_message = ""
        ...
        #pylint: disable=broad-except
            ...
            exception_message = exception_raised.__str__()
        self.assertEqual(exception_message, "Vaccination date must be posterior to current date")

    def test_wrong_date_format(self):
        """la fecha de vacunacion tiene un formato incorrecto"""
        file_test = JSON_FILES_RF2_PATH + "test_ok.json"
        vac_date = "20220301"
        exception_message = ""
        my_manager= VaccineManager()
        ...
        # pylint: disable=broad-except
            ...
            exception_message = exception_raised.__str__()
        self.assertEqual(exception_message, "Invalid date format")
