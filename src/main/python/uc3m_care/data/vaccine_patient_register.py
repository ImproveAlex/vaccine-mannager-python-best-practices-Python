...
from uc3m_care.storage.patients_json_storage import PatientsJsonStore
...
class VaccinePatientRegister:
    ...
    def create_patient_from_patient_system_id( cls, patient_system_id ):
        ...
        if patient_found is None:
            raise VaccineManagementException(ErrorMsg.NOT_FOUND_PATIENT_SYSTEM_ID.value)

        # set the date when the patient was registered for checking the md5
        freezer = freeze_time(
            datetime.fromtimestamp(patient_found[KeyLabel.VPR_TIME_STAMP.value]).date())
        freezer.start()
        patient = cls(patient_found[KeyLabel.VPR_PATIENT_ID.value],
                      patient_found[KeyLabel.VPR_FULL_NAME.value],
                      patient_found[KeyLabel.VPR_REGISTRATION_TYPE.value],
                      patient_found[KeyLabel.VPR_PHONE_NUMBER.value],
                      patient_found[KeyLabel.VPR_AGE.value])
        freezer.stop()
        if patient.patient_system_id != patient_system_id:
            raise VaccineManagementException(ErrorMsg.MANIPULATED_PATIENT_DATA.value)
